...
std::unique_ptr<FossFit::TimerService<FossFit::Windows>> timer_service;
...
namespace FossFit::platform_specific
{
    void initWindows(std::unique_ptr<flutter::MethodChannel<>> channel) {
        ...
        timer_service = std::make_unique<FossFit::TimerService<FossFit::Windows>>();
        /*
        FossFit::platform_specific::nativeCodeInit<FossFit::Windows, NotifyActionCallback>({
    ...
    void timer_method_call_handler(const flutter::MethodCall<>& call, const std::unique_ptr<flutter::MethodResult<>>& result) {
        if (call.method_name() == "timer") FossFit::handleMethodCall<Windows, MethodCall::Timer, const flutter::MethodCall<>*, flutter::MethodResult<>*>(&call, result.get());
        else if (call.method_name() == "add") FossFit::handleMethodCall<Windows, MethodCall::Add, const flutter::MethodCall<>*, flutter::MethodResult<>*>(&call, result.get());
        else if (call.method_name() == "stop") FossFit::handleMethodCall<Windows, MethodCall::Stop, const flutter::MethodCall<>*, flutter::MethodResult<>*>(&call, result.get());
        else {
            FossFit::platform_specific::sendResult<Windows, flutter::MethodResult<>*, false>(result.get());
        }
    }
    ...
    TimerArgs getTimerArgs<Windows>(const flutter::MethodCall<>* call, flutter::MethodResult<>* result) {
        ...
        if (const auto timestampValue = args.find(flutter::EncodableValue("timestamp")); timestampValue != args.end()) {
            timestamp = FossFit::convertLongToTimePoint(std::get<int64_t>(timestampValue->second));
        }
    ...
    std::optional<std::chrono::time_point<fclock_t>> getAddArgs<Windows>(const flutter::MethodCall<>* call, flutter::MethodResult<>* result) {
        ...
        {
            return FossFit::convertLongToTimePoint(std::get<int64_t>(timestamp->second));
        }
        return std::nullopt;
    }
