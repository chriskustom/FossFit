...
namespace FossFit {
    ...
    class FossFitTimer {
    public:
        explicit FossFitTimer(std::chrono::milliseconds timerDuration);
        ~FossFitTimer();

        static inline FossFitTimer<P> emptyTimer() { return FossFitTimer<P>(0ms); }
    ...
    template<Platform P>
    bool FossFitTimer<P>::hasSecondsUpdated() {
    ...
    template<Platform P>
    FossFitTimer<P>::FossFitTimer(std::chrono::milliseconds msTimerDuration) : state(TimerState::Paused),
                                                                              timerDuration(msTimerDuration),
                                                                              totalTimerDuration(msTimerDuration) {
    }

    template<Platform P>
    FossFitTimer<P>::~FossFitTimer() {
        stop();
    }

    template<Platform P>
    void FossFitTimer<P>::start(std::chrono::milliseconds elapsedTime) {
    ...
    template<Platform P>
    void FossFitTimer<P>::increaseDuration(std::chrono::milliseconds increase) {
