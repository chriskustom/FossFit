package com.kustom.FossFitfork
...
class TimerService : Service() {
    ...
    var FossFitTimer: FossFitTimer = FossFitTimer.emptyTimer()
    ...
    private val stopReceiver =
        object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                ...
                FossFitTimer.stop(applicationContext)
                FossFitTimer.expire()
    ...
    private val addReceiver =
        object : BroadcastReceiver() {
            override fun onReceive(context: Context?, intent: Intent?) {
                ...
                if (FossFitTimer.isExpired()) return startTimer(
                    FossFitTimer.ONE_MINUTE_MILLI,
                    timeStamp ?: 0
                )

                FossFitTimer.increaseDuration(applicationContext, FossFitTimer.ONE_MINUTE_MILLI)
                updateNotification(FossFitTimer.getRemainingSeconds())
    ...
    private fun onTimerExpired() {
        Log.d("TimerService", "onTimerExpired duration=${FossFitTimer.getDurationSeconds()}")
        FossFitTimer.expire()
        vibrate()
        playSound()
        notifyFinished()
    ...
    private fun getDelay(startTime: Long): Long {
        ...
        val delay = FossFitTimer.getRemainingMillis() % 1000
    ...
    private fun startTimer(msDuration: Long, timeStamp: Long) {
        ...
        FossFitTimer.stop(applicationContext)
        FossFitTimer = FossFitTimer(msDuration)
        FossFitTimer.start(
            applicationContext,
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.UPSIDE_DOWN_CAKE) {
            startForeground(
                ONGOING_ID,
                getProgress(FossFitTimer.getRemainingSeconds()).build(),
                FOREGROUND_SERVICE_TYPE_SPECIAL_USE
            )
        } else {
            startForeground(ONGOING_ID, getProgress(FossFitTimer.getRemainingSeconds()).build())
        }

        Log.d("TimerService", "onTimerStart seconds=${FossFitTimer.getDurationSeconds()}")

        timerRunnable = object : Runnable {
            override fun run() {
                val startTime = SystemClock.elapsedRealtime()
                if (FossFitTimer.isExpired()) return
                if (FossFitTimer.hasSecondsUpdated()) updateNotification(FossFitTimer.getRemainingSeconds())
    ...
    private fun getProgress(timeLeftInSeconds: Int): NotificationCompat.Builder {
        ...
        val notificationBuilder = NotificationCompat.Builder(this, channelId)
            ...
            .setProgress(FossFitTimer.getDurationSeconds(), timeLeftInSeconds, false)
