...
class MainActivity : FlutterActivity() {
    ...
    override fun configureFlutterEngine(flutterEngine: FlutterEngine) {
        ...
        channel?.setMethodCallHandler { call, result ->
            when (call.method) {
                ...
                "getProgress" -> {
                    if (timerBound && timerService?.FossFitTimer?.isRunning() == true)
                        result.success(
                            intArrayOf(
                                timerService?.FossFitTimer!!.getRemainingSeconds(),
                                timerService?.FossFitTimer!!.getDurationSeconds()
                            )
                        )
                ...
                "add" -> {
                    if (timerService?.FossFitTimer?.isRunning() == true) {
        ...
        object : BroadcastReceiver() {
            override fun onReceive(context: Context, intent: Intent) {
                channel?.invokeMethod(
                    "tick",
                    timerService?.FossFitTimer?.generateMethodChannelPayload()
                )
            }
        }
    ...
    companion object {
        const val FLUTTER_CHANNEL = "com.kustom.FossFitfork/android"
