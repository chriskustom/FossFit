...
#include "FossFit_native_impl.h"
#include <iostream>
#include <filesystem>
#include <libnotify/notify.h>

namespace FossFit::platform_specific
{


    struct FossFitApplicationData {
        FossFitApplicationData() : methodChannel(nullptr), notification(nullptr) {}

        FossFitApplicationData(FlMethodChannel* channel) : iconPath(std::filesystem::canonical("/proc/self/exe").parent_path() += "/data/flutter_assets/assets/ic_launcher.png"),
                                                           methodChannel(channel),
                                                           notification(nullptr),
                                                           timer_service(std::make_unique<FossFit::TimerService<FossFit::Linux>>())
                                                           {}
        std::string iconPath;
        FlMethodChannel* methodChannel;
        NotifyNotification *notification;
        std::unique_ptr<FossFit::TimerService<FossFit::Linux>> timer_service;
    };

    FossFitApplicationData applicationData;

    void initLinux(FlMethodChannel* channel) {
        applicationData = FossFitApplicationData(channel);
        FossFit::platform_specific::nativeCodeInit<FossFit::Linux, NotifyActionCallback>({
    ...
    void timer_method_call_handler(FlMethodChannel* flChannel, FlMethodCall* method_call, gpointer) {
        if (strcmp(fl_method_call_get_name(method_call), "timer") == 0) FossFit::handleMethodCall<Linux, MethodCall::Timer, FlMethodChannel*, FlMethodCall*>(flChannel, method_call);
        else if (strcmp(fl_method_call_get_name(method_call), "add") == 0) FossFit::handleMethodCall<Linux, MethodCall::Add, FlMethodChannel*, FlMethodCall*>(flChannel, method_call);
        else if (strcmp(fl_method_call_get_name(method_call), "stop") == 0) FossFit::handleMethodCall<Linux, MethodCall::Stop, FlMethodChannel*, FlMethodCall*>(flChannel, method_call);
        else {
            FossFit::platform_specific::sendResult<Linux, FlMethodCall*, false>(method_call);
        }
    }
    ...
    TimerArgs getTimerArgs<Linux>(FlMethodChannel*, FlMethodCall* methodCall) {
        ...
        if (timestampValue != nullptr && fl_value_get_type(timestampValue) == FL_VALUE_TYPE_INT) {
            timestamp = FossFit::convertLongToTimePoint(fl_value_get_int(timestampValue));
        }
    ...
    std::optional<std::chrono::time_point<fclock_t>> getAddArgs<Linux>(FlMethodChannel*, FlMethodCall* methodCall) {
        ...
        {
            return FossFit::convertLongToTimePoint(fl_value_get_int(timestamp));
        }
        return std::nullopt;
    }
